...
import { Plus, TrendingUp, TrendingDown, LineChart as StockIcon } from "lucide-react"; // Added icons
...
export default function MyStocksPage() {
  ...
  const { totalStockPnL, totalStockCost, totalStockValue } = React.useMemo(() => {
    if (isLoading) return { totalStockPnL: 0, totalStockCost: 0, totalStockValue: 0 };
    ...
    const stockInvestments = investments.filter(inv => {
        ...
        const security = listedSecurities.find(ls => ls.symbol === (inv as StockInvestment).tickerSymbol);
        if (!security) return false; // Exclude if no matching listed security (e.g., delisted)
        return security.securityType === 'Stock' || (security.securityType === 'Fund' && isStockRelatedFund(security.fundType));
    }) as StockInvestment[];


    // Aggregate investments by ticker symbol
    const aggregatedBySymbol: { [key: string]: { totalShares: number, totalCost: number, symbol: string } } = {};
    stockInvestments.forEach(inv => {
      if (inv.tickerSymbol) {
        if (!aggregatedBySymbol[inv.tickerSymbol]) {
          aggregatedBySymbol[inv.tickerSymbol] = { totalShares: 0, totalCost: 0, symbol: inv.tickerSymbol };
        }
        aggregatedBySymbol[inv.tickerSymbol].totalShares += inv.numberOfShares || 0;
        aggregatedBySymbol[inv.tickerSymbol].totalCost += (inv.numberOfShares || 0) * (inv.purchasePricePerShare || 0);
      }
    });
    
    Object.values(aggregatedBySymbol).forEach(agg => {
      const security = listedSecurities.find(ls => ls.symbol === agg.symbol);
    ...
    return { totalStockPnL: pnlSum, totalStockCost: costSum, totalStockValue: valueSum };
  }, [investments, listedSecurities, isLoading]);

  const totalStockPnLPercent = totalStockCost > 0 ? (totalStockPnL / totalStockCost) * 100 : (totalStockValue > 0 ? Infinity : 0);
  const isTotalStockProfitable = totalStockPnL >= 0;

  const formatCurrency = (value: number | undefined, currencyCode: string = "EGP") => { // Assuming EGP as default for overall summary for now
    ...
    return new Intl.NumberFormat('en-EG', { style: 'currency', currency: currencyCode, minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(value);
  };
  ...
  return (
    <div className="space-y-8 relative min-h-[calc(100vh-10rem)]">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-foreground">My Stocks & Equity Funds</h1>
        <p className="text-muted-foreground">Overview of your stock and equity fund investments.</p>
      </div>
      <Separator />
      ...
      {isLoading ? (
        ...
        <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Total Stocks & Equity Funds P/L</CardTitle>
                {isTotalStockProfitable ? <TrendingUp className="h-4 w-4 text-accent" /> : <TrendingDown className="h-4 w-4 text-destructive" />}
            </CardHeader>
            <CardContent>
                <div className={cn("text-2xl font-bold", isTotalStockProfitable ? "text-accent" : "text-destructive")}>
                    {isMobile ? formatCurrencyWithSuffix(totalStockPnL) : formatCurrency(totalStockPnL)}
                </div>
                <p className="text-xs text-muted-foreground">
                    {totalStockPnLPercent === Infinity ? '∞' : totalStockPnLPercent.toFixed(2)}% overall P/L
                </p>
