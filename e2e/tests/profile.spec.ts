...
test.describe('Profile Management', () => {
  ...
  test('should toggle password visibility', async ({ page }) => {
    ...
    const isPasswordUser = await page.locator('[data-testid="password-input"]').count() > 0;
  ...
  test('should show upload status when uploading image', async ({ page }) => {
    ...
    const isPasswordUser = await page.locator('[data-testid="image-upload-input"]').count() > 0;
