...
test.describe("Dashboard Page", () => {
  ...
  test("should load dashboard with all components", async ({ page }) => {
    ...
    await expect(
      page.getByTestId("investment-distribution-chart"),
    ).toBeVisible();
    await expect(
      page.getByTestId("monthly-investment-distribution-chart"),
    ).toBeVisible();
    await expect(
      page.getByTestId("investment-breakdown-section"),
    ).toBeVisible();
  });
  ...
  test("should display numeric values in summary cards", async ({ page }) => {
    ...
    const totalInvested = await page
      .getByTestId("total-invested-card")
      .locator("p:not(.text-xs)")
      .first()
      .textContent();
    expect(totalInvested).toMatch(CURRENCY_AMOUNT_PATTERN);

    const totalRealizedPL = await page
      .getByTestId("total-realized-pl-card")
      .locator("p:not(.text-xs)")
      .first()
      .textContent();
    expect(totalRealizedPL).toMatch(CURRENCY_AMOUNT_PATTERN);

    const currentPortfolioPL = await page
      .getByTestId("current-portfolio-pl-card")
      .locator("p:not(.text-xs)")
      .first()
      .textContent();
    expect(currentPortfolioPL).toMatch(CURRENCY_AMOUNT_PATTERN);

    const cashBalance = await page
      .getByTestId("total-cash-balance-card")
      .locator("p:not(.text-xs)")
      .first()
      .textContent();
  ...
  test("should display monthly cash flow values", async ({ page }) => {
    ...
    const currentIncome = await page
      .getByTestId("current-income-card")
      .locator("p:not(.text-xs)")
      .first()
      .textContent();
    ...
    const totalIncome = await page
      .getByTestId("total-income-card")
      .locator("p:not(.text-xs)")
      .first()
      .textContent();
    ...
    const totalExpenses = await page
      .getByTestId("total-expenses-card")
      .locator("p:not(.text-xs)")
      .first()
      .textContent();
    ...
    const totalInvestments = await totalInvestmentsCard
    ...
    const remainingCash = await page
      .getByTestId("remaining-cash-card")
      .locator("p:not(.text-xs)")
      .first()
      .textContent();
  ...
  test("should recalculate summary when recalculate button is clicked", async ({
    page,
  }) => {
    // Define a pattern that matches currency-formatted numbers (e.g., "EGP 920,000.000" or "-EGP 1,234.56")
    const CURRENCY_AMOUNT_PATTERN = /^-?[A-Za-z\s]*[\s-]*[\d,.]+/;
    ...
    const initialTotalInvested = await page
      .getByTestId("total-invested-card")
      .locator("p:not(.text-xs)")
      .first()
      .textContent();
    ...
    const recalculatedTotalInvested = await page
      .getByTestId("total-invested-card")
      .locator("p:not(.text-xs)")
      .first()
      .textContent();
  ...
  test("should navigate to cash flow details when view details is clicked", async ({
    page,
  }) => {
    // Wait for the page to load completely
    // await page.waitForLoadState("networkidle");
    ...
    await expect(
      page.getByRole("heading", { name: /cash flow/i }),
    ).toBeVisible();
  });
  ...
  test("should display charts with data", async ({ page }) => {
    ...
    const monthlyChart = page.getByTestId(
      "monthly-investment-distribution-chart",
    );
    await expect(monthlyChart).toBeVisible();
  });

  test("should display investment breakdown cards with correct data", async ({
    page,
  }) => {
    // Wait for the dashboard to be fully loaded by checking for a key element
    await expect(page.getByTestId("dashboard-page")).toBeVisible();

    // Wait for the investment breakdown section and its content to be visible
    ...
    // Wait for at least one investment card to be present
    await expect(
      breakdownSection.locator('div[class*="grid"] > div').first(),
    ).toBeVisible({ timeout: 10000 });

    // Get all investment type cards
    const investmentCards = breakdownSection.locator(
      'div[class*="grid"] > div',
    );
    ...
    for (let i = 0; i < cardCount; i++) {
      ...
      // Check for investment type icon (first SVG in the card header)
      await expect(
        card.locator("div.flex.items-center.gap-3 svg").first(),
      ).toBeVisible();

      // Check for investment type name
      const typeName = await card
        .locator('span[class*="text-md"][class*="font-bold"]')
        .textContent();
      expect(typeName).toBeTruthy();

      // Check for P/L text in the card
      const plDiv = card
        .locator('div:has(> div:has-text("P/L"))')
        .filter({ hasText: /P\/L/ });
      await expect(plDiv.first()).toBeVisible();
      const plText = await plDiv.first().textContent();
      expect(plText).toMatch(/P\/L\s*[\d.]+%?/);

      // Check for invested amount
      const investedLabel = card.getByText("Invested");
      await expect(investedLabel).toBeVisible();
      const investedAmount = await investedLabel
        .locator("xpath=following-sibling::div[1]")
        .textContent();
      // Match currency format like "EGP 920K" or "EGP 1,234.56"
      expect(investedAmount).toMatch(/^[A-Za-z\s]*[\d,.]+[KMBT]?$/);

      // Check for current value
      const currentLabel = card.getByText("Current");
      await expect(currentLabel).toBeVisible();
      const currentValue = await currentLabel
        .locator("xpath=following-sibling::div[1]")
        .textContent();
      // Match currency format like "EGP 920K" or "EGP 1,234.56"
      expect(currentValue).toMatch(/^[A-Za-z\s]*[\d,.]+[KMBT]?$/);

      // Check for portfolio percentage
      const portfolioLabel = card.getByText("% of Portfolio");
      await expect(portfolioLabel).toBeVisible();
      const portfolioPercent = await portfolioLabel
        .locator("xpath=following-sibling::div[1]")
        .textContent();
